USE biblioteca;
    ...
    -- quantitat d'usuaris que estan a prestecs pero no a la taula usuaris!!!
	SELECT count(p.usuario_id)
    FROM usuarios AS u
    RIGHT JOIN prestamos AS p ON u.usuario_id = p.usuario_id
    WHERE u.usuario_id is NULL; 
    
	SELECT count(p.usuario_id)
	FROM usuarios AS u
	INNER JOIN prestamos AS p ON u.usuario_id = p.usuario_id;
